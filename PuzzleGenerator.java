...
class PuzzleGenerator implements PuzzleSource {
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            int nFound;
            nFound = 0;
            int seqNum = sq.sequenceNum();
            if (sq.successor() == null && sq.direction() != 0) {
                for (int i = 0; i < sq.successors().size(); i++) {
                    Sq check = model.get(sq.successors().get(i));
                    int seqNum1 = check.sequenceNum();
                    if (sq.connectable(check)) {
                        if (seqNum1 != 0 && seqNum != 0
                                && seqNum1 == (seqNum + 1)) {
                            found = check;
                            nFound = 1;
                            break;
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (int i = 0; i < sq.predecessors().size(); i++) {
                    Sq check = model.get(sq.predecessors().get(i));
                    int seqNum1 = check.sequenceNum();
                    if (check.connectable(sq)) {
                        if (seqNum1 != 0 && seqNum != 0
                                && seqNum1 == (seqNum - 1)) {
                            found = check;
                            nFound = 1;
