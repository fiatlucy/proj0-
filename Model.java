...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq now = _board[x][y];
                if (now._successors == null) {
                    now._successors = new PlaceList();
                }
                if (now._predecessors == null) {
                    now._predecessors = new PlaceList();
                }
                for (int i = 0;
                     i < _allSuccessors[x][y][now.direction()].size(); i++) {
                    Place next = _allSuccessors[x][y][now.direction()].get(i);
                    Sq sqnext = _board[next.x][next.y];
                    now._successors.add(next);
                    if (sqnext._predecessors == null) {
                        sqnext._predecessors = new PlaceList();
                    }
                    sqnext._predecessors.add(now.pl);
                }
            }
        }
        _unconnected = last - 1;
    ...
    void makeBoard() {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                int k = _solution[x][y];
                if (k == _width * _height) {
                    _board[x][y] = new Sq(x, y, k, true, 0, 0);
                } else if (k == 1) {
                    _board[x][y] = new Sq(x, y, k, true,
                            _solnNumToPlace[k].dirOf(_solnNumToPlace[ k + 1]),
                            0);
                } else {
        ...
        if (_allSquares.size() != last) {
            throw badArgs("_allSquares.size() is not last");
        }
    }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq now = _board[x][y];
                if (now._sequenceNum == _width * _height) {
                    if (model._board[x][y]._predecessor != null) {
                        now._predecessor =
                                get(model._board[x][y]._predecessor.x,
                                model._board[x][y]._predecessor.y);
                    }
                } else if (now._sequenceNum == 1) {
                    if (model._board[x][y]._successor != null) {
                        now._successor = get(model._board[x][y]._successor.x,
                                model._board[x][y]._successor.y);
                    }
                } else {
                    if (model._board[x][y]._predecessor != null) {
                        now._predecessor =
                ...
                if (model._board[x][y]._head != null) {
                    now._head = get(model._board[x][y]._head.x,
                            model._board[x][y]._head.y);
                }
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq now = _board[x][y];
                if (now._successor == null && now.sequenceNum() != size()
                        && now.sequenceNum() != 0) {
                    for (int i = 0; i < now.successors().size(); i++) {
                        Sq next = get(now.successors().get(i));
                        if (next.sequenceNum() != 0 && now.connectable(next)) {
                            now.connect(next);
                            ans = true;
                            break;
                        }
    ...
    void solve() {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq now = _board[x][y];
                now._group = 0;
                now._sequenceNum = _solution[x][y];
                if (now._sequenceNum == 1) {
                    now._predecessor = null;
                    now._successor = get(_solnNumToPlace[now._sequenceNum + 1]);
                } else if (now._sequenceNum == size()) {
                    now._successor = null;
                    now._predecessor =
                            get(_solnNumToPlace[now._sequenceNum - 1]);
                } else {
                    now._successor = get(_solnNumToPlace[now._sequenceNum + 1]);
                    now._predecessor =
                             get(_solnNumToPlace[now._sequenceNum - 1]);
    ...
    private int arrowDirection(int x, int y) {
        int num0 = _solution[x][y];
        Place seq0 = _solnNumToPlace[num0];
        Place seq1 = _solnNumToPlace[num0 + 1];
        if (get(seq0)._successor != null) {
            return seq0.dirOf(seq1);
        }
        return 0;
    }
    ...
    final class Sq {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (_successor == null && _predecessor == null
        ...
        /** assigns this predecessor, the @param next square successor,
         * and group of both. */
        void assign(Sq next) {
